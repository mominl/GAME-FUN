...
// Remove Node.js TextEncoder import, use browser's built-in version
...
// Helper function to safely encode strings without relying on TextEncoder
function safeTextEncode(text: string): Uint8Array {
  // Direct ASCII encoding - handles the "metadata" string safely for Metaplex
  // This avoids the TextEncoder compatibility issue
...
export async function createToken(params: CreateTokenParams): Promise<string> {
  ...
  try {
    ...
    // When the IPFS functionality is restored, we'll need to properly integrate this    // Use our safe encode function to avoid TextEncoder issues
    const metadataBytes = safeTextEncode('metadata');
    console.log('Using safe encoding for "metadata" string:', metadataBytes);
